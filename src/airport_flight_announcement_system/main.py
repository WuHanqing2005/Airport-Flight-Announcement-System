...
# Version: 2025.11.11 (Web + Flask + SocketIO) - Cache Clearing Feature
# Copyright: Wu Hanqing
# Unauthorized use is prohibited. Infringement will be prosecuted.
#
# CHANGE LOG (v2.2):
# - FEATURE: Added 'Clear Cache' button and corresponding backend logic to delete all files in the 'output' directory.
# - CRITICAL FIX: Resolved `greenlet.error` by ensuring eventlet.monkey_patch() is called first.
# - CRITICAL FIX: Replaced `threading.Event` with `eventlet.event.Event` to prevent threading conflicts.
# - CRITICAL FIX: Updated event handling logic to use the correct eventlet API (`.ready()`, `.send()`, and re-instantiation).
#
# Notes:
...
Quick Start:
1) Ensure Python dependencies are installed (pydub, flask, flask-socketio, eventlet, pandas, openpyxl).
...
class AirportAnnouncementSystem:
    ...
    - stop_event (eventlet.event.Event): An event used to request cancellation of long-running tasks.
    ...
    Notes:
    - Uses eventlet.event.Event for greenlet-friendly synchronization (do not replace with threading.Event).
    ...
    def __init__(self):
        ...
        # Use eventlet's Event for safe green-thread synchronization
    ...
    def update_progress(self, step, total, text):
        ...
        - Yields control to eventlet for cooperative multitasking.
        ...
        try:
            ...
            # Yield control to the eventlet hub to allow other operations to run
            eventlet.sleep(0.01)
        except Exception:
    ...
    def convert_to_stereo(self, input_folder):
        ...
        for i, path in enumerate(all_files, start=1):
            if self.stop_event.ready():  # Use .ready() for eventlet.event.Event
    ...
    def stop_all_audio(self):
        ...
        Behavior:
        - Sends stop signal by .send() on the eventlet Event if not already set.
        ...
        if not self.stop_event.ready():
            self.stop_event.send()  # Use .send() to trigger the eventlet Event
    ...
    def search_and_synthesize(self, flight_key, announce_type):
        ...
        # Create a new, fresh event for this specific task. This is the correct pattern for eventlet.
        self.stop_event = eventlet.event.Event()
        ...
        for idx, wav_file in enumerate(segment_paths, start=1):
            if self.stop_event.ready():  # Use .ready() for eventlet
